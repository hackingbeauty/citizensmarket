...
describe SessionsController do
  fixtures        :users
  before do 
    @user  = mock_model(User, :role_symbols => [:admin])
    #mock_model(User, :role_symbols => [:admin, :contributor])
    #@current_user = mock_user
    @login_params = { :email=> 'quentin@blah.com', :password => 'test' } #=> changed field from :login to :email
    User.stub!(:authenticate).with(@login_params[:email], @login_params[:password]).and_return(@user)
    
  end
  def do_create#_for_contributor
    post :create, @login_params
    
  end
  #def do_create_for_administrator
  #  User.stub!(:authenticate).with(@login_params[:email], @login_params[:password]).and_return(@administrator)
  #  post :create, @login_params
  #end
    ...
    [ [:nil,       nil,            nil],
      ...
      [ true, false ].each do |want_remember_me|
        describe "my request cookie token is #{has_request_token.to_s}," do
          describe "and ask #{want_remember_me ? 'to' : 'not to'} be remembered, it" do 
            before do
            ...
            it "greets me nicely" do 
              do_create 
              response.flash[:message].should =~ /success/i   
            end
