module AssociatedFormHelper
  ...
  def add_associated_link(name, object, opts = {})
    ...
    function = "if (typeof #{variable} == 'undefined') #{variable} = 0;
                ...
                $('.star_rating:last').children('.star-rating').rating();"
    ...
    unless associated.empty?
      ...
      associated.map do |element|
        fields_for_associated(element, (opts[:fields_for] || {}).merge(:name => name)) do |f|
          @template.render({:partial => "#{partial}", :locals => {local_assign_name.to_sym => element, :f => f, :review_issue_counter => 3 }.merge(opts[:locals] || {})}.merge(opts[:render] || {}))
        end
      end
    end
  end
